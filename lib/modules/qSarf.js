...
//var qSarf = (function(){
	...
	var SARF_SAGHEER_HEADER = 'Verbs_VerbNo,Verbs_Words_Corpus_Root_English,Verbs_Type,Verbs_class,Verbs_Sura,Verbs_Verse,Meaning-English,Meaning-Urdu,Count,perfect,Imperfect,Imperative,Active-participle,Passive-participle,Verbal-noun,root,type,class',
		...
		verbForms = ['', 'I', 'II', 'III', 'IV', 'V', 'VI', 'VII', 'VIII', 'IX', 'X', 'XI', 'XII'], //used to map verb form types
...
'205,qwm,d10,اِسْتَغْفَرَ,1,6,to be straight;  to act straight,ثابت قدم رهنا,47,اِسْتَقَامَ, يَسْتَقِيمُ, اِسْتَقِمْ, مُسْتَقِيم, -, اِسْتِقَامَة,ق و م,10,اِسْتَغْفَرَ',
	...
	function lookup(verb){
		if(!repo.final){ prepareData(); }
		if(!verb || verb === ''){
			return format( repo.final ); //return everything. 
		}
		else{
			//return data for matching verb only
		}
	}
	...
	function format(data){
		return _.map(data, function( o ){
			return {
				'verb': o.perfect,
				'root': o.rootAr,
				'form': verbForms[ +o.type ], //todo: map it
				'frequency': +o.count,
				'translation': o.en,
				'class': o.class
			};
	...
	publicApi = {
		...
		//comment below after API is stable
		repo: repo,
	};
	return publicApi;
//})();
